...
(function ($) {
   ...
   */
		...
		if (window.location.hash.length > 0 && phaseTasks !== null) {
			$('body').addClass('psp-task-expanded');
		}

		if ($('body').hasClass('psp-task-expanded') || $('body').hasClass('psp-dashboard-page')) {

			// Check if we only want to expand a specific one
			if (window.location.hash.length > 0 && $('body').hasClass('psp-task-expanded') && phaseTasks !== null) {

				var phaseIndex = phaseTasks[1];

				$('.psp-phase.phase-' + (parseInt(phaseIndex) + 1) + ' .psp-task-list').show().addClass('active');
				$('.psp-phase.phase-' + (parseInt(phaseIndex) + 1) + ' a.task-list-toggle').addClass('active');
			} else {

				// Toggle the task list on or off
				$('.psp-task-list').show().addClass('active');
				$('a.task-list-toggle').addClass('active');
			}

			$(document).trigger('psp-tasks-auto-expanded');
		}
   ...
   */
		...
		$('#psp-offcanvas-menu a').smoothScroll();

		$("#psp-offcanvas-menu").mmenu({
			"offCanvas": {
				"position": "right"
			}
		});

		var nav_menu = $('#psp-offcanvas-menu').data('mmenu');
		$('#psp-projects #nav-menu a').click(function (e) {
			e.preventDefault();
			nav_menu.open();
		});
		...
		$('.psp-archive-project').click(function (e) {
			e.preventDefault();
		...
		$('.psp-modal-btn').leanModal({ closeButton: '.modal-close' });
		$('[data-toggle="psp-tooltip"]').tooltip();
   ...
   *
   */
		if ($('.psp-match-height-row').length) {
			$('.psp-match-height-item').matchHeight();
		}
		if ($('.psp-phase-info').length) {
			$('.psp-phase-info').matchHeight();
		}
		if ($('.psp-overview-col').length) {
			$('.psp-overview-col').matchHeight();
		}
   ...
   */
		...
		if ($('.psp-masonry').length) {

			var $grid = $('.psp-masonry').masonry({
				// options
				itemSelector: '.psp-masonry-item',
   ...
   */
		...
		jQuery('.doc-list-toggle').click(function (e) {
			...
			target = jQuery(this).parent().siblings('.psp-phase-documents-wrapper');
		...
		$('.psp-expand-comments').click(function (e) {
		...
		$(document).on('click', '.all-checkbox', function () {
			...
			if ($(this).is(':checked')) {
				$(this).parents('.psp-document-form').find('input.specific-checkbox').prop('checked', false);
				$(this).parents('.psp-document-form').find('ul.psp-notify-list input').prop('checked', true);
				$(this).parents('.psp-document-form').find('ul.psp-notify-list').slideUp('fast');
			} else {
				$(this).parents('.psp-document-form').find('ul.psp-notify-list input').prop('checked', false);
			}
		});
		...
		$(document).on('click', '.specific-checkbox', function () {
			...
			if ($(this).is(':checked')) {
				$(this).parents('.psp-document-form').find('ul.psp-notify-list').slideDown('fast');
				$(this).parents('.psp-document-form').find('input.all-checkbox').prop('checked', false);
			} else {
				$(this).parents('.psp-document-form').find('ul.psp-notify-list').slideUp('fast');
				$(this).parents('.psp-document-form').find('ul.psp-notify-list input').prop('checked', false);
			}
   ...
   */
		...
		$('.psp-documents-row li').each(function () {
		...
		$(document).on('click', '.psp-documents-row .doc-status', function (e) {
			...
			$('#psp-document-status-modal').find('input[name="psp-document-id"]').val(data.id);
			$('#psp-document-status-modal').find('input[name="psp-document-name"]').val(data.title);
			$('#psp-document-status-modal select[name="doc-status"]').val(data.status);

			$(document).trigger('psp-before-document-status-modal-open', [data]);

			$('#psp-document-status-modal').show();
		});

		$(document).on('click', '.modal-close', function (e) {
			$('#psp-document-status-modal').hide();
			psp_document_status_reset();
		});

		// Reset the modal
		$(document).on('click', '.js-psp-doc-status-reset', function (e) {
			psp_document_status_reset();
		});

		// Modal reset
		function psp_document_status_reset() {
			$('#psp-document-status-modal input[type="checkbox"]').prop('checked', false);
			$('#psp-document-status-modal textarea').val('');
			$('#psp-document-status-modal .psp-message-form').hide();
			$('#psp-document-status-modal .psp-confirm-note').hide();
			$('#psp-document-status-modal .psp-document-form').show();
			$('#psp-document-status-modal .pano-modal-actions').show();
			$('#psp-document-status-modal .psp-notify-list').hide();
		...
		$(document).on('submit', '.document-update-dialog form', function (event) {
			...
			var data = {
				project_id: $(this).find('input[name=psp-project-id]').val(),
				doc_id: $(this).find('input[name=psp-document-id]').val(),
				filename: $(this).find('input[name=psp-document-name]').val(),
				editor: $(this).find('input[name=psp-current-user]').val(),
				status: $(this).find('select[name="doc-status"]').val(),
				message: $(this).find('textarea[name="psp-doc-message"]').val(),
				users: []
			};

			var status_label = $(this).find('select[name="doc-status"] option:selected').text();
			var elm = $('#psp-project-' + data.project_id + '-doc-' + data.doc_id);

			var in_phase = false;
			if ($(elm).parents('.psp-phase').length) {
				in_phase = true;
				var phase_id = $(elm).parents('.psp-phase').data('phase-index');
			}

			// Build the notification list
			$(this).find('input.psp-notify-user-box:checked').each(function () {
			...
			$.ajax({
				...
				success: function success(response) {
					...
					$('#psp-project-' + data.project_id + '-doc-' + data.doc_id + ' a.doc-status').removeClass().addClass('doc-status').addClass('status-' + data.status).html(status_label + ' <span class="fa fa-pencil" href="#"></span>');
      ...
      *
      */
					$('#psp-document-status-modal .psp-message-form').fadeIn();
					if (data.users.length) {
						$('#psp-document-status-modal .psp-confirm-note').fadeIn();
					}

					setTimeout(function () {
						$('#psp-document-status-modal').fadeOut();
						psp_document_status_reset();
					}, 1000);

					if (in_phase) {
						psp_update_phase_approved_documents_count(phase_id, data.project_id);
					}

					// Update global stats
					//
					$('#psp-stat-documents h3 span').text(response.data.approved);
		...
		function psp_update_phase_approved_documents_count(phase_id, post_id) {

			var ajaxurl = $('#psp-ajax-url').val();

			$.ajax({
				url: ajaxurl + '?action=psp_get_phase_approval_count',
				type: 'post',
				data: {
					post_id: post_id,
					phase_id: phase_id
		...
		$('.task-table-edit-link').click(function (e) {
			e.preventDefault();
			$(this).parents('.psp-task-table-body').find('.task-select').fadeIn('slow');
		});

		$('.psp-task-table').on('click', '.complete-task-link', function (e) {
			...
			psp_update_task_progress(post_id, phase_id, task_id, progress);

			var the_parent = $(this).parents('.task-row');
			target = $(the_parent).find('.psp-progress-bar em');
			...
			if ($('.psp-task-project').length) {

				$(target).parent().attr('data-progress', progress);
				psp_update_my_task_count(project_id);
			}
		...
		$('#psp-phases').on('click', '.complete-task-link', function (e) {
			...
			// Update dynamically
			psp_update_task_progress(project_id, phase_id, task_id, progress, phase_progress, total_progress);
		...
		$('.psp-task-table').on('click', '.task-save-button', function (e) {
			...
			psp_update_task_progress(project_id, phase_id, task_id, progress);

			$(the_parent).find('span').children('em').removeClass().addClass('status').addClass('psp-' + progress);
		...
		$('.psp-my-tasks').on('click', '.task-save-button', function (e) {
			...
			psp_update_task_progress(project_id, phase_id, task_id, progress, null, total_progress);
		...
		$('.psp-my-tasks').on('click', '.complete-task-link', function (e) {
		...
		$('#psp-phases').on('click', '.task-save-button', function (e) {
		...
		$('.psp-task-project').on('click', '.task-edit-link', function (e) {
		...
		$('#psp-phases').on('click', '.task-edit-link', function (e) {
		...
		$('table.psp-table-pagination').each(function () {
			...
			var $pager = $('<div class="psp-pager">' + psp_js_label_more + '</div>');
		...
		if ($('#psp-documents-list .psp-documents-row > li').length > 5) {

			var currentPage = 0;
			var numPerPage = 5;
			var $table = $('#psp-documents-list .psp-documents-row');
			$table.bind('repaginate', function () {
		...
		$(document).on('submit', '.psp-comment-form', function (e) {
			...
			var phase = $(this).parents('.psp-phase');
			...
			if ($(this).find('.psp-comment-status').length < 0) {
				$(this).prepend('<div class="psp-comment-status"></div>');
			}

			var status_div = $(this).children('.psp-comment-status');

			status_div.html('<p>' + psp_translate_processesing + '</p>');
			...
			$.ajax({
				...
				error: function error(XMLHttpRequest, textStatus, errorThrown) {
					...
					status_div.html('<p class="psp-ajax-error">' + psp_translate_comment_failed + '</p>');
				},
				success: function success(data, textStatus) {
					if (data == 'success' || textStatus == 'success') {

						status_div.html('<p class="psp-ajax-success">' + psp_translate_comment_success + '</p>');
						...
						if ($(form).parents('.phase-comments-wrapper, .task-comments-wrapper').children('ol.commentlist').length > 0) {
							...
							$(form).parents('.phase-comments-wrapper, .task-comments-wrapper').find('li.psp-hide').slideDown('slow');
						} else {
						...
						$(document).trigger('psp-comment-added', [form]);
					} else {

						status_div.html('<p class="psp-ajax-error">' + psp_translate_comment_error + '</p>');
					}
				}
		...
		$(document).on('click', '#psp-comments a.comment-reply-link', function (e) {
		...
		if ($('body').hasClass('psp-pdf-view')) {

			setTimeout(function () {

				var area = $('#psp-projects');
		...
		function getCanvas(area) {
			...
			var height = $('#psp-projects').prop('scrollHeight');
...
function psp_update_total_progress(projectID) {
	...
	jQuery.ajax({
		...
		success: function success(new_progress) {
			...
			if (jQuery('.psp-task-project-' + projectID).length) {
				jQuery('.psp-task-project-' + projectID + ' .psp-progress span').removeClass().addClass('psp-' + new_progress).html('<b>' + new_progress + '%</b>');
				return;
			}

			jQuery('.psp-progress span').removeClass().addClass('psp-' + new_progress).html('<b>' + new_progress + '%</b>');

			psp_update_milestone_progress(new_progress);
		},
		error: function error(data) {
			console.log(data);
		}
...
function psp_update_milestone_progress(progress) {
...
function psp_update_phase_completion(projectID, phaseID) {
	...
	jQuery('#phase-' + phaseID + ' ul.psp-task-list').children('li.task-item').each(function () {
...
function psp_update_task_progress(project_id, phase_id, task_id, progress, phase_progress, total_progress) {
...
window.psp_update_my_task_count = function (project_id) {
	var phase_id = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;


	var total_tasks = jQuery('.psp-task-project-' + project_id + ' .psp-element-tally-all').data('count');
	var total_completed_tasks = 0;
	var started_tasks = 0;

	jQuery('.psp-task-project-' + project_id + ' li.task-item, .psp-single-' + project_id + ' li.task-item').each(function () {
	...
	jQuery('.psp-task-project-' + project_id + ' .psp-element-tally-started strong').html(started_tasks);
	jQuery('.psp-task-project-' + project_id + ' .psp-element-tally-completed strong').html(total_completed_tasks);

	// Applies only to Project Single
	if (jQuery('body').hasClass('psp-single')) {
		...
		if (phase_id !== null) {
			...
			jQuery('.psp-single-' + project_id + ' .phase-' + phase_id + ' li.task-item').each(function () {
			...
			jQuery('.psp-single-' + project_id + ' .phase-' + phase_id + ' .task-list-toggle b.tasks-completed, .psp-single-' + project_id + ' .phase-' + phase_id + ' .task-count .completed').html(phase_completed_tasks);

			var phase_total_tasks = jQuery('.psp-single-' + project_id + ' .phase-' + phase_id + ' .task-count .total').text();

			// Update the Phase Chart
			psp_update_my_phase_count(project_id);
		}

		jQuery('#psp-stat-tasks h3 span').html(total_completed_tasks);

		total_tasks = jQuery('#psp-stat-tasks h3').text().match(/[^\/]+?$/)[0].trim();
 ...
 * @returns		{void}
 */
function psp_update_my_phase_count(project_id) {

	var completed_phases = 0,
	    total_phases = jQuery('#psp-stat-phases h3').text().match(/[^\/]+?$/)[0].trim();

	if (jQuery('.psp-single-' + project_id + ' #psp-phases').attr('data-phase-auto') == 'No') {
		return false; // Bail, we aren't updating the Phase progress based on Complete Tasks
	}

	jQuery('.psp-single-' + project_id + ' .psp-phase').each(function (phase_index, phase) {
	...
	jQuery('#psp-stat-phases h3 span').html(completed_phases);
...
(function ($) {
	...
	$(document).ready(function () {
		...
		var pspajaxurl = $('#psp-ajax-url').val();

		$("#psp-offcanvas-task-details").mmenu({
			"offCanvas": {
				"position": "right"
			},
			"navbar": {
		...
		var $offCanvas = $('#psp-offcanvas-task-details'),
		    taskDetailsPanel = $('#psp-offcanvas-task-details').data('mmenu');

		$(document).on('click', '.js-open-task-panel', function (e) {
			$(this).parents('li').find('a.psp-task-title').click();
		});

		$(document).on('click', 'a.psp-task-title', function (event) {
			...
			data['assigned_name'] = data['assigned_name'] ? data['assigned_name'] : $taskItem.find('.psp-assigned-to').first().text().trim();
			data['due_date'] = data['due_date'] ? data['due_date'] : $taskItem.find('.psp-task-due-date').first().text().trim();

			var statusClasses = $taskItem.find('.psp-progress-bar em').attr('class');
			...
			$offCanvas.find('.status').removeClass().addClass('status').addClass('psp-' + data['status']);
			...
			$('.task-panel-tabs-content .tabs-panel').each(function (index, tab) {
				...
				$.ajax({
					...
					success: function success(response, _success) {
						...
						$(document).trigger('psp-task-panel-tab-loaded', [id, data]);
					},
			...
			$(document).trigger('psp-before-task-panel-open', [data, $offCanvas]);

			taskDetailsPanel.open();
		});

		$(document).on('psp-task-panel-tab-loaded', function (event, id, data) {
		...
		$(document).on('psp-comment-added', function (event, $form) {

			// Only run on Task Comments
			if ($form.parents('#psp-offcanvas-task-details').length <= 0) return;

			$('#discussions-count').html(parseInt($('#discussions-count').text()) + 1);

			// This is a little more complicated
			var $offCanvas = $form.parents('#psp-offcanvas-task-details').first(),
			    data = $offCanvas.data('openTaskData');

			// Single, Dashboard, then Your Tasks view
			var $outsideOfPanelCommentCount = $('.psp-single-' + data['project'] + ' .phase-' + (parseInt(data['phase_index']) + 1) + ' .task-item-' + data['task_index'] + ' .psp-task-discussions .text, .psp-task-project-' + data['project'] + ' .psp-tasks-phase[data-phase_id="' + data['phase_id'] + '"]' + ' .task-item-' + data['task_index'] + ' .psp-task-discussions .text, .psp-task-table .task-row[data-task_id="' + data['task_id'] + '"] .psp-tmi-comments .text');

			// Ensure it is visible if it was previously 0
			$outsideOfPanelCommentCount.parent().prev('.psp-separator').removeClass('hidden');
		...
		$(document).on('psp-before-document-status-modal-open', function (event, data) {
