...
add_action( 'psp_save_post', 'psp_process_project_progress', 10, 2 );
function psp_process_project_progress( $post_id, $post ) {
    ...
    if( $new_progress != $current_progress ) {
        ...
        do_action( 'psp_project_progress_change', $post_id, $new_progress );
    }

	if( $new_progress > $current_progress ) {
		// Progress has moved forward so we fire an acction for the current progress
		do_action( 'psp_project_completion', $post_id, $new_progress );
	}
...
add_action( 'psp_project_completion', 'psp_fire_project_completed_hook', 10, 2 );
function psp_fire_project_completed_hook( $post_id, $new_progress ) {
...
add_action( 'psp_save_post', 'psp_mark_as_complete', 10 , 2 );
...
function psp_update_project_quicklinks( $views ) {
	...
	if( $post_counts[ 'publish' ] > 0) {
		$views['publish'] = '<a class="' . $publish_class . '" href="edit.php?post_status=publish&post_type=psp_projects">Published <span class="count">(' . $post_counts[ 'publish' ] . ')</span></a>';
	}

	if( $post_counts[ 'draft' ] > 0) {
		$views['draft'] = '<a class="' . $draft_class . '" href="edit.php?post_status=draft&post_type=psp_projects">Draft <span class="count">(' . $post_counts[ 'draft' ] . ')</span></a>';
	}

	if( $post_counts[ 'trash' ] > 0) {
		$views['trash'] = '<a class="' . $trash_class . '" href="edit.php?post_status=trash&post_type=psp_projects">Trash <span class="count">(' . $post_counts[ 'trash' ] . ')</span></a>';
	}
